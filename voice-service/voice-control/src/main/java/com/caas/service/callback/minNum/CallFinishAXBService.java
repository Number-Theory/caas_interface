...
public class CallFinishAXBService extends DefaultServiceCallBack {
	...
	public void callService(ChannelHandlerContext ctx, ServiceRequest request, ServiceResponse response, Map<String, Object> paramsObject) {
		...
		if(StringUtil.isBlank(billUrl)) {
			...
			Long callTime = 0L;
			try {
				callTime = DateUtil.getTime(finishModel.getReleasetime(), "yyyy-MM-dd HH:mm:ss") - DateUtil.getTime(finishModel.getStarttime(), "yyyy-MM-dd HH:mm:ss");
			...
			String billingUrl = ConfigUtils.getProperty("billingUrl", String.class); // TODO
			try {
