 ...
 */
@Service
public class CallFinishAXService extends DefaultServiceCallBack {

	private static final Logger logger = LogManager.getLogger(CallFinishAXService.class);
	...
	public void callService(ChannelHandlerContext ctx, ServiceRequest request, ServiceResponse response, Map<String, Object> paramsObject) {
		...
		logger.info("【AX话单回调接口】接收到广西话单回调请求内容：{}", finishModel);
		...
		if (StringUtil.isBlank(billUrl)) {
			...
			if ("0".equals(finishModel.getCalltype())) {
				safetyCallBillModel.setFlag("0");
			} else if ("1".equals(finishModel.getCalltype())) {
