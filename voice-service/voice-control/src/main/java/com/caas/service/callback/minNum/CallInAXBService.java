 ...
 */
@Service
public class CallInAXBService extends DefaultServiceCallBack {
	private static final Logger logger = LogManager.getLogger(CallInAXBService.class);
	...
	public void callService(ChannelHandlerContext ctx, ServiceRequest request, ServiceResponse response, Map<String, Object> paramsObject) {
		...
		logger.info("【AXB状态回调接口】接收到广西状态回调请求内容：{}", callinModel);
		...
		if(StringUtil.isBlank(statusUrl)) {
			...
			safetyCallStatusModel.setCalleeDisplayNum(callinModel.getTelZ());
			safetyCallStatusModel.setCaller(callinModel.getTelB());
			safetyCallStatusModel.setCallId((String)orderBindMap.get("requestId"));
			if("20".equals(callinModel.getCalltype())) {
				safetyCallStatusModel.setCallStatus("failed");
			} else {
				safetyCallStatusModel.setCallStatus("invite");
			}
			safetyCallStatusModel.setCallTime(callinModel.getCalltime());
